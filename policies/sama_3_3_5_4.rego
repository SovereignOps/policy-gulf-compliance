...
# Requirement: Ensure compliance with 3.3.5.4 for aws_iam_user, google_project_iam_member, azurerm_role_assignment, oci_identity_user
...
deny[msg] {
    input.resource_type == "aws_iam_user"
    not allow
    msg := sprintf("AWS resource '%v' does not comply with SAMA Control 3.3.5.4", [input.resource_name])
}

deny[msg] {
    input.resource_type == "google_project_iam_member"
    not allow
    msg := sprintf("Google resource '%v' does not comply with SAMA Control 3.3.5.4", [input.resource_name])
}

deny[msg] {
    input.resource_type == "azurerm_role_assignment"
    not allow
    msg := sprintf("Azure resource '%v' does not comply with SAMA Control 3.3.5.4", [input.resource_name])
}

deny[msg] {
    input.resource_type == "oci_identity_user"
    not allow
    msg := sprintf("OCI resource '%v' does not comply with SAMA Control 3.3.5.4", [input.resource_name])
}
